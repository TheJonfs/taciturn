...
describe('probe template independence (the market-town enabler)', () => {
  const riverRidge = firstBattleBeat(getNode(M1_CAMPAIGN_GRAPH, M1_NODES.riverRidge).beats)!.battle;
...
describe('probeBattleFor', () => {
  it("prefers the node's own battlefield when it has one", () => {
    ...
    expect(probe.template).toBe(firstBattleBeat(node.beats)!.battle.template);
  });
