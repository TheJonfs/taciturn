...
- **Elevation/water-table convention.** Tiles at elevation 0 are deep water (move cost 3, rendered as deep water visually); elevation 1 is shallow water (move cost 2, rendered as shallow water); elevation 2+ is land (move cost 1). Convention is universal across all maps; future terrain-manipulation effects modify elevation, with water effects emerging automatically from the elevation alone. **Note (Session 33, ADR-0073):** the implementation authors `water_shallow` and `water_deep` as *distinct terrain types* (in addition to elevation), with both carrying the `'water'` tag in the ruleset's terrain registry. The original design called for "elevation alone determines water-ness; no separate water terrain type needed." The shift to distinct terrain types was made at S33 plan time to keep terrain identity orthogonal to elevation (so future elevation-mutation abilities don't accidentally re-tag water as land), and the tag abstraction in ADR-0073 keeps cross-ability composition (Tidewalker / Float) compact regardless. See ADR-0073 for the full rationale.
