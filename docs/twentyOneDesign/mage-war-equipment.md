...
**Rasp Pendant**
- Effect: 10% of final damage dealt is converted to MP drain (wielder gains, target loses)
- Class restriction: none
- Notes: Resource attrition accessory. Damage output reduced by 10%; target loses MP equal to that 10%; wielder gains the same. Most effective on high-damage builds — Knight War Axe (~76 expected dmg) drains ~7-8 MP per hit, depleting a Mage's spell budget in 4-5 hits. Standard guardrails: drain caps at target's current MP (no negative MP), doesn't apply to KO'd targets. Damage conversion happens on *final damage dealt* (after resistance modifiers apply). May require additional engine plumbing to insert at the end of the damage pipeline cleanly. (See Engine Requirements.)
