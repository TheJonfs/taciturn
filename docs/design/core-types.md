...
- **Multi-layer is LIVE (S96, ADR-0155)** — the original "validated to layer 0 only" constraint is superseded. The validator now enforces the multi-layer consistency rules instead: a layer ≥ 1 tile (a DECK — bridge span/platform) requires a layer-0 tile beneath it, must clear it by `BRIDGE_MIN_CLEARANCE` (2), and v1 caps the stack at one deck layer. Alvera Village's western bridge is the first shipped deck. Decks are destroyable by Worldcraft (permanently — see the ADR) and occlude LoS only in their 1-thick band.
