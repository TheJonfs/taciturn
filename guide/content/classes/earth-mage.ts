// Earth Mage — the instructor's hand-authored prose for the
// Specialization spread. Mechanical values flow in from ../src/content
// at build time; this file holds only the voice.
//
// Ability-note keys match the ids the catalog resolves for the Earth
// Mage: attack, earth_strike, earth_blessing, earth_curse, earth_quake,
// earth_cataclysm (actives); earth_resilience, earth_communion,
// bedrock_stride (passives).
...
find that the Earth Mage's slows, blinds, and silences compound into
...
range, and more often than not it leaves the target labouring through
unseen mud besides. One opens most engagements with it and is rarely
sorry.`,
      compact: 'Reliable arc-range damage; usually slows the target too. The opener.',
    },
    earth_blessing: {
      full: `Not every spell is aimed at an enemy. Earth's Blessing settles
a slow, certain mending upon an ally — Regen does not hurry, but it does
not miss, and across a long engagement it pays for itself many times
over.`,
      compact: 'Lays Regen on an ally — unhurried, certain healing over time.',
    },
    earth_curse: {
      full: `Earth Curse asks two separate questions of a target: whether
...
without spending a point of its health.`,
    ...
    earth_quake: {
      full: `The first of the Earth Mage's area spells. It is not the
heaviest blow in the discipline, but it finds everything standing in the
diamond, and it leaves the survivors slow. Against a cluster, that is
worth more than a larger number against one.`,
...
Expensive, slow to arrive, and, when it arrives, decisive.`,
    ...
    earth_resilience: {
      full: `A reaction. A blow that finds the Earth Mage teaches her feet
something — struck, she gains ground. It is the discipline's quiet
answer to being closed upon: hurt her, and she is harder to pin the next
turn for it.`,
      compact: 'Reaction: a blow that lands on her grants her movement. Her answer to being closed on.',
    },
    earth_communion: {
      full: `The quiet engine of the whole discipline. Earth Communion
weights every status the Earth Mage attempts — her slows, her blinds,
her silences all take hold more often for it. If the cadet equips one
...
drop — the Earth Mage steps off any ledge as though it were the next
...
threats is no small gift.`,
  ...
  strategy: `The Earth Mage rewards the patient hand and punishes the
hurried one. She is slow — do not fight that, build around it. Earth
Communion belongs in her Support slot before anything else; it is the
multiplier the rest of her kit is balanced against. From there, the
pattern is simple to state and hard to do well: open with Earth Strike,
read which foe most needs unmaking, and curse or quake accordingly,
...
It will. It always does.`,
  ...
  marginalia: [
    'The cadet who rushes the Earth Mage forward has not understood the Earth Mage.',
    'Communion before Cataclysm. Every term I say it; every term someone learns it the hard way.',
    'Earth’s Blessing is not glamorous. Win a long engagement without it, then we’ll talk.',
